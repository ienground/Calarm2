            ...
            if (datas.isNotEmpty()) {
                when (sharedPreferences.getString(SharedKey.CALARM_CREATE_OPTION, SharedDefault.CALARM_CREATE_OPTION)) {
                    SharedValue.CALARM_CREATE_FIRST_EVENT -> {
                        createCalarm(context, datas.first())
                    }
                    SharedValue.CALARM_CREATE_FIXED_TIME_EVENT -> {
                        for (data in datas) {
                            val calendar = Calendar.getInstance().apply { timeInMillis = data.time }
                            if (calendar.get(Calendar.HOUR_OF_DAY) * 60 + calendar.get(Calendar.MINUTE) < sharedPreferences.getInt(SharedKey.FIXED_TIME, SharedDefault.FIXED_TIME)) {
                                createCalarm(context, data)
                            }
                        }
                    }
                    SharedValue.CALARM_CREATE_ALL_EVENTS -> {
                        for (data in datas) {
                            createCalarm(context, data)
                        }
                    }
                ...
                if (row.length() != 0) {
                    ...
                    if (item.length() != 0) {
                        ...
                        if (element.length() != 0) {
                            ...
                            GlobalScope.launch(Dispatchers.IO) {
                                val entity = calarmDatabase?.getDao()?.getByDataId(data.dataId) ?: CalarmEntity(data.dataId, data.calendarId, true, data.address, SharedDefault.HOME_LATITUDE, SharedDefault.HOME_LONGITUDE, "", defaultRingtone, true)
                                entity.let {
                                    val calendarEvent = getCalendarEventByID(context, data.dataId).first()
                                    it.time = calendarEvent.startDate
                                    it.label = calendarEvent.title
                                }
                                calarmDatabase?.getDao()?.add(entity)
                                val subEntity = subCalarmDatabase?.getDao()?.getAutoGenerated(data.dataId) ?: SubCalarmEntity(data.dataId, value + sharedPreferences.getInt(SharedKey.READY_TIME, SharedDefault.READY_TIME), true, value)
